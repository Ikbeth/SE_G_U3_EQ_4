...
from Proyecto_U3 import Modulos as M
#import Valor
#import Atributo
#import Nodo
import math
...
def id3s(vectorP):
    ...
    def generarVector(listaAtributos, vector):  # Vector = registro/caso/fila
        ...
        for i in range(len(vector)):
            aux.append(M.Valor.Valor(listaAtributos[i].nombre, vector[i]))
        return aux
    ...
    def removerAtributo(caso, nombre):
        ...
        for j in range(len(caso)):  # Por cada caso
            if M.Valor.Valor(nombre) in caso[j]:
                indiceColumna = caso[j].index(M.Valor.Valor(nombre))
    ...
    def ID3(casos, listaAtributos, mayoritaria):
        ...
        contadorNodos += 1
        root = M.Nodo.Nodo(contadorNodos)
        mismaClase = False
        aux = -1
        clase = listaAtributos[len(listaAtributos) - 1];
        for k in range(len(clase.listaEtiquetas)):  # por cada clase
            ...
            if (len(casos) == 0):
                ...
                for i in range(len(A.listaEtiquetas)):  # Para cada etiqueta (v) del atributo hacer
                    if (len(A.listaCasos) == 0):
                        ...
                        mayoritaria = claseMayoritaria(A.listaCasos,
                                                       listaAtributos[len(listaAtributos) - 1].listaEtiquetas)
    ...
    ####################################################################################################
    # EJECUCIÓN PRINCIPAL
    archivo = open("../Archivos/instanciaDiscretizada.csv")
    ...
    for i in range(len(aux)):
        listaAtributos.append(M.Atributo.Atributo(aux[i]))
    ...
    toEvaluar = generarVectorToEval(nombresAtributos, evaluar)
    print(evaluar)
    ...
    claseAs = "Clase Asignada: " + temp.getEtiqueta()
    print(claseAs)
